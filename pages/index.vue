      ...
      >
        <div class="flex flex-col h-full">
          ...
          <div class="flex flex-col justify-between flex-1 p-3">
            ...
            <button
              @click="sellItem(item)"
              :disabled="!item.steam_price || isItemOnCooldown(item)"
              class="w-full bg-[#FFB823] text-black py-1 rounded hover:bg-yellow-400 transition mt-2 disabled:bg-gray-700 disabled:text-gray-400 disabled:cursor-not-allowed relative"
            >
              <span v-if="isItemOnCooldown(item)">
                {{ formatCooldown(item.nextAnnounce) }}
              </span>
              <span v-else>Sotish</span>
            </button>
          </div>
        </div>
      </div>
